...
export function PatientFAQ() {
  ...
  return (
    <section id="faq" style={{ padding: "80px 0", background: C.white }}>
      <div className="wrap">
        <div className="mob-stack" style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 60, alignItems: "start" }}>
          <FadeIn>
            <div>
              <div style={{ color: C.teal, fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>FAQ</div>
              <h2 style={{ fontSize: "clamp(24px, 3vw, 36px)", fontWeight: 800, color: C.text, lineHeight: 1.2, marginBottom: 18 }}>
                Your Questions,<br />Answered
              </h2>
              <Link href="/contact-us" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: C.teal, color: "#fff", padding: "11px 22px", borderRadius: 6, fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {items.map((item, i) => (
              <div key={i} onClick={() => setOpen(open === i ? null : i)} style={{ background: open === i ? C.tealSoft : C.light, borderRadius: 12, padding: "18px 24px", cursor: "pointer", border: `1px solid ${open === i ? C.teal + "40" : C.grayLight}`, transition: "all 0.3s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
                  <span style={{ fontWeight: 600, fontSize: 14, color: C.text }}>{item.q}</span>
                ...
                <div style={{ maxHeight: open === i ? 200 : 0, overflow: "hidden", transition: "max-height 0.4s ease" }}>
                  <div style={{ paddingTop: 12, color: C.textLight, fontSize: 14, lineHeight: 1.7 }}>{item.a}</div>
                </div>
              </div>
            ))}
          </div>
